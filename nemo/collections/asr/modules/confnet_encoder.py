...
__all__ = ['ConfnetEncoder']


class ConfnetEncoder(NeuralModule, Exportable):
    """
    The encoder for ASR model of Confnet.
    Based on this paper:
    'Confnet: Convolution-augmented Transformer for Speech Recognition' by Anmol Gulati et al.
    https://arxiv.org/abs/2005.08100

    Args:
        feat_in (int): the size of feature channels
        n_layers (int): number of layers of ConfnetBlock
    ...
    def forward(self, audio_signal, length=None):
        ...
        for lth, layer in enumerate(self.layers):
            audio_signal = layer(x=audio_signal, pad_mask=pad_mask)
